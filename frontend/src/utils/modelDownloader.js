...
class ModelDownloader {
  ...
  async initializeTfBackend() {
    try {
      ...
      console.warn(
        "Failed to initialize WebGL backend, falling back to CPU:",
        error
      );
      try {
        ...
        await tf.ready();
        console.log(
          "TensorFlow.js is using CPU backend, version:",
          tf.version.tfjs
        );
      } catch (cpuError) {
    ...
    try {
      ...
      try {
        ...
        return new Promise((resolve) => {
          ...
          request.onerror = () => {
            console.error(
              `Error checking if model ${modelId} is downloaded:`,
              request.error
            );
            resolve(false);
          };
  ...
  async downloadModel(modelId, progressCallback = null) {
    try {
      ...
      try {
        ...
        const modelModule = await this.dynamicallyImportModel(
          model.importInfo.package
        );

        if (!modelModule) {
          throw new Error(
            `Failed to import model package ${model.importInfo.package}`
          );
        }
        ...
        switch (modelId) {
          case "speech-commands": {
            ...
            const recognizer =
              modelModule[model.importInfo.creator]("BROWSER_FFT");
        ...
        if (modelInstance) {
          ...
          if ("save" in modelInstance) {
            ...
            console.warn(
              "Model does not have direct save method, storing metadata only"
            );
          }
        }
  ...
  async deleteModel(modelId) {
    try {
      ...
      try {
        ...
        console.warn(
          `Error removing TensorFlow model from IndexedDB: ${error}`
        );
      ...
      return new Promise((resolve, reject) => {
        ...
        getRequest.onerror = () => {
          console.error(
            `Error checking model ${modelId} before deletion:`,
            getRequest.error
          );
          this.modelStatus.set(modelId, "error");
  ...
  async loadModel(modelId) {
    try {
      ...
      if (!isDownloaded) {
        console.warn(
          `Model ${modelId} not downloaded, attempting to download now`
        );
        ...
        return {
          ...
          predict: async (input) => {
            try {
              ...
              console.error(
                `Error during prediction with model ${modelId}:`,
                error
              );
              throw error;
            }
      ...
      return new Promise((resolve) => {
        ...
        request.onsuccess = () => {
          ...
          const totalSize = downloadedModels.reduce(
            (total, model) => total + (model.size || 0),
            0
          );
